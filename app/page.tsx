...
export default function Home() {
  ...
  const generatePDFReceipt = (orderData: any) => {
    ...
    doc.text('Fixés avant expédition', 150, 180);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text('Tarifs indicatifs livraison : Cotonou 1000F | Akpakpa 1500F | Calavi 500F | Lomé 500-1500F | Abidjan 1000-2000F', 20, 215);
    doc.text('Contacts : +229 01 69 32 55 76 / +225 07 104 106 04 / +226 04 26 18 02 / +228 92 04 66 86', 20, 222);
    doc.text('SAB MIDLEY — Abomey-Calavi, Bénin | Ce reçu fait office de justificatif officiel.', 20, 230);
  ...
  return (
    <div className="min-h-screen bg-[#090A0C] text-slate-100 font-sans relative">
      ...
      {/* ================= MODALE / PAGE DE SUCCÈS DE COMMANDE RECYCLÉE ================= */}
      {orderSuccess && (
        <div className="py-16 px-6 max-w-4xl mx-auto">
          <div className="p-8 md:p-12 rounded-3xl bg-slate-900 border border-[#D4AF37]/40 shadow-2xl space-y-8 relative overflow-hidden">
            
            {/* Image d'illustration en haut (Livreur en route vers le magasin / expédition) */}
            <div className="relative h-64 rounded-2xl overflow-hidden border border-[#D4AF37]/30 shadow-inner">
              <img 
                src="https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=1200&q=80" 
                alt="Livreur SAB MIDLEY en route" 
                className="w-full h-full object-cover"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-[#090A0C] via-transparent to-transparent" />
              <div className="absolute bottom-4 left-6 right-6 flex items-center justify-between">
                <span className="px-3 py-1.5 rounded-lg bg-[#090A0C]/90 text-[#D4AF37] border border-[#D4AF37]/30 text-xs font-bold backdrop-blur-md">
            ...
            <div className="text-center space-y-3">
              <h2 className="text-3xl md:text-4xl font-extrabold text-white">Merci pour votre commande !</h2>
              <p className="text-slate-300 text-base max-w-2xl mx-auto">
                Votre paiement a bien été reçu. Un reçu officiel a été téléchargé automatiquement sur votre appareil. Vous pouvez également en télécharger un autre ci-dessous à tout moment.
              </p>
            </div>

            {/* Explications du fonctionnement de la livraison */}
            <div className="p-6 rounded-2xl bg-[#090A0C] border border-slate-800 space-y-4">
              <h3 className="text-lg font-bold text-[#D4AF37]">Comment fonctionne la livraison ?</h3>
              <p className="text-sm text-slate-300 leading-relaxed">
                Notre équipe logistique va vous contacter très prochainement par téléphone pour organiser la livraison de votre article <strong className="text-white">({orderSuccess.productTitle})</strong> à <span className="text-white underline">{orderSuccess.city}</span>.
              </p>
              
              <div className="pt-2 border-t border-slate-800 space-y-2">
                <span className="text-xs font-bold uppercase tracking-wider text-[#D4AF37] block">Tarifs indicatifs de livraison par zone :</span>
                <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-slate-300">
                  <li>• Cotonou : <strong className="text-white">1 000 F</strong></li>
                  <li>• Akpakpa : <strong className="text-white">1 500 F</strong></li>
                  <li>• Abomey-Calavi : <strong className="text-white">500 F</strong></li>
                  <li>• Lomé : <strong className="text-white">500 F - 1 500 F</strong></li>
                  <li>• Abidjan (petits articles) : <strong className="text-white">1 000 F à 2 000 F</strong></li>
                </ul>
                <p className="text-xs text-[#D4AF37] italic pt-1">
                  * Le prix fixe exact de la livraison vous sera communiqué et validé avec vous avant toute expédition.
                </p>
              </div>

              <div className="pt-2 border-t border-slate-800 text-xs text-slate-400 space-y-1">
                <p className="font-semibold text-slate-200">Nos numéros officiels d'assistance :</p>
                <div className="flex flex-wrap gap-3 font-mono text-white">
                  <span>🇧🇯 +229 01 69 32 55 76</span>
                  <span>🇨🇮 +225 07 104 106 04</span>
                  <span>🇧🇫 +226 04 26 18 02</span>
                  <span>🇹🇬 +228 92 04 66 86</span>
                </div>
              </div>
            </div>

            {/* Récapitulatif rapide de la commande */}
            <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 text-xs space-y-1 text-slate-300">
              <p><strong className="text-white">Client :</strong> {orderSuccess.fullName} ({orderSuccess.phone})</p>
              <p><strong className="text-white">Article :</strong> {orderSuccess.productTitle} — <span className="text-[#D4AF37] font-bold">{orderSuccess.price}</span></p>
              <p><strong className="text-white">Ville de destination :</strong> {orderSuccess.city}</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              <button
                ...
                className="flex-1 py-4 rounded-xl bg-[#D4AF37] text-[#090A0C] font-bold text-sm hover:bg-[#c5a030] transition shadow-lg shadow-[#D4AF37]/20 text-center"
              >
                Télécharger un autre reçu (PDF)
              </button>
              <button
                onClick={() => setOrderSuccess(null)}
                className="flex-1 py-4 rounded-xl bg-slate-800 text-white font-semibold text-sm hover:bg-slate-700 transition border border-slate-700 text-center"
              >
      ...
      {/* ================= MODALE / FORMULAIRE DE COMMANDE ARTICLE ================= */}
      {orderProduct && !orderSuccess && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm overflow-y-auto">
          <div className="bg-slate-900 border border-[#D4AF37]/40 rounded-3xl max-w-lg w-full p-6 md:p-8 space-y-6 relative shadow-2xl my-8">
            <button 
            ...
            <div className="space-y-2">
              <span className="text-[#D4AF37] text-xs font-bold uppercase tracking-wider">Finalisation de commande</span>
              <h3 className="text-2xl font-bold text-white">Commander cet article</h3>
              <p className="text-xs text-slate-300">{orderProduct.title} — <span className="text-[#D4AF37] font-bold">{orderProduct.price}</span></p>
            </div>
            ...
            <form onSubmit={handleOrderSubmit} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  required
                  className="w-full px-4 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white focus:border-[#D4AF37] outline-none text-sm"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    value={formData.city}
                    onChange={handleInputChange}
                    placeholder="Cotonou, Abidjan, Lomé..."
                    required
                    className="w-full px-4 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white focus:border-[#D4AF37] outline-none text-sm"
                  />
                </div>
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-1">Adresse exacte de livraison <span className="text-[#D4AF37]">*</span></label>
                <input 
                  type="text" 
                  name="address"
              ...
              <div>
                {!showPartnerField ? (
                  ...
                  >
                    + Vous avez un code partenaire ou conseiller ?
                  </button>
                ) : (
                  <div>
              ...
              >
                Procéder au paiement sécurisé ({orderProduct.price})
              </button>
            </form>
          </div>
        </div>
        ...
        <>
          ...
          <section id="carrieres" className="py-24 px-6 max-w-7xl mx-auto border-t border-[#D4AF37]/20">
            <div className="text-center space-y-4 mb-16">
              ...
              <p className="text-slate-400 max-w-2xl mx-auto text-lg">
                Développez votre activité au sein de notre réseau commercial en pleine expansion.
              </p>
            </div>
            {/* ... formulaire carrières préservé ... */}
          </section>
        </>
      )}

      {/* Pied de page */}
      <footer className="border-t border-[#D4AF37]/20 py-12 px-6 bg-[#090A0C] text-center text-xs text-slate-500">
