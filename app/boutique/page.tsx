    ...
    <div className="min-h-screen bg-[#090A0C] text-slate-100 font-sans relative">
      ...
      <header className="sticky top-0 z-40 bg-[#090A0C]/90 backdrop-blur-md border-b border-[#D4AF37]/20">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <nav className="flex items-center space-x-8 text-sm font-medium text-slate-300">
            <a href="/" className="hover:text-[#D4AF37] transition">Accueil</a>
            <a href="/boutique" className="text-[#D4AF37] font-bold">Boutique & Négoce</a>
            <a href="/partenaire" className="hover:text-[#D4AF37] transition">Portail Partenaire</a>
          </nav>
        </div>
      </header>
      ...
      {orderSuccess && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-emerald-500/50 rounded-3xl p-8 max-w-lg w-full text-center space-y-6 shadow-2xl relative">
            <button onClick={() => setOrderSuccess(null)} className="absolute top-4 right-4 bg-slate-800 text-white px-3 py-1.5 rounded-full text-xs font-bold">✕</button>
            ...
            <div className="flex space-x-3 pt-2">
              <button onClick={() => generatePDFReceipt(orderSuccess)} className="flex-1 py-3.5 bg-[#D4AF37] text-[#090A0C] font-bold rounded-xl text-xs">Télécharger le reçu PDF</button>
              <button onClick={() => setOrderSuccess(null)} className="px-5 py-3.5 bg-slate-800 text-white font-semibold rounded-xl text-xs">Fermer</button>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-[#D4AF37]/40 rounded-3xl p-6 md:p-8 max-w-2xl w-full relative space-y-6 my-8">
            <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 bg-slate-800 text-white px-3.5 py-1.5 rounded-full text-xs font-bold">✕</button>
            <div className="h-72 rounded-2xl overflow-hidden bg-slate-950 border border-slate-800">
              <img src={selectedProduct.images?.[activeImageIndex] || selectedProduct.image} alt={selectedProduct.title} className="w-full h-full object-cover" />
            </div>
            ...
            <button onClick={() => { setSelectedProduct(null); setOrderProduct(selectedProduct); setActiveImageIndex(0); }} className="w-full py-3.5 bg-[#D4AF37] text-[#090A0C] font-bold rounded-xl text-xs shadow-lg">
      ...
      {orderProduct && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-slate-900 border border-[#D4AF37]/40 rounded-3xl p-6 md:p-8 max-w-xl w-full relative space-y-6 my-8">
            <button onClick={() => setOrderProduct(null)} className="absolute top-4 right-4 bg-slate-800 text-white px-3.5 py-1.5 rounded-full text-xs font-bold">✕</button>
            <h2 className="text-xl font-extrabold text-white">Finaliser l'achat: {orderProduct.title} ({orderProduct.price})</h2>
            <form onSubmit={handleOrderSubmit} className="space-y-4 text-xs">
              <div>
                ...
                <input type="text" name="fullName" value={formData.fullName} onChange={handleInputChange} placeholder="Ex: Koffi Mensah" required className="w-full px-3.5 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-semibold uppercase text-slate-400 mb-1">Téléphone (Mobile Money) *</label>
                  <input type="text" name="phone" value={formData.phone} onChange={handleInputChange} placeholder="+229..." required className="w-full px-3.5 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white outline-none" />
                </div>
                <div>
                  <label className="block font-semibold uppercase text-slate-400 mb-1">Email</label>
                  <input type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="email@example.com" className="w-full px-3.5 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white outline-none" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-semibold uppercase text-slate-400 mb-1">Pays *</label>
                  <select name="country" value={formData.country} onChange={handleInputChange} className="w-full px-3.5 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white outline-none">
                ...
                <div>
                  ...
                  <input type="text" name="city" value={formData.city} onChange={handleInputChange} placeholder="Ex: Abidjan / Cotonou" required className="w-full px-3.5 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white outline-none" />
                </div>
              </div>
              <div>
                <label className="block font-semibold uppercase text-slate-400 mb-1">Adresse exacte *</label>
                <input type="text" name="address" value={formData.address} onChange={handleInputChange} placeholder="Ex: Quartier, Rue..." required className="w-full px-3.5 py-3 rounded-xl bg-[#090A0C] border border-slate-800 text-white outline-none" />
              </div>
              <button type="submit" className="w-full py-4 bg-[#D4AF37] text-[#090A0C] font-bold rounded-xl shadow-lg">
          ...
          {productsList.map((product: any) => (
            <div key={product.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden flex flex-col justify-between hover:border-[#D4AF37]/40 transition shadow-xl">
              <div className="h-60 overflow-hidden relative bg-slate-950">
                ...
                <div className="absolute top-4 right-4 bg-[#090A0C]/90 px-3 py-1 rounded-lg text-sm font-bold text-[#D4AF37] border border-[#D4AF37]/30">
                  {product.price}
                </div>
              </div>
